...
try:
    from koyeb import ServicesApi, CreateService, DeploymentDefinition, DockerSource
except ImportError as e:
    msg = (
        "Dask Cloud Provider Koyeb requirements are not installed.\n\n"
        "Please either conda or pip install as follows:\n\n"
        "  conda install -c conda-forge dask-cloudprovider       # either conda install\n"
        '  pip install "dask-cloudprovider[koyeb]" --upgrade       # or python -m pip install'
    )
...
class Service:
    """A superclass for managing Koyeb Services
    Parameters
    ----------

    client: Callable[str]
        Function to create an client on demand
        These will be used to interact with the Koyeb API.

    app_id: str
        The App id to launch service in.

    deployment_definition: DeploymentDefinition
        The definiion of the service that this service should use to launch
        itself.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._client = client
        self.app_id = app_id
        self.deployment_definition = deployment_definition
    ...
    async def _update_service(self):
    ...
    async def start(self):
        timeout = Timeout(60, "Unable to start %s after 60 seconds" % self.service_type)
        while timeout.run():
            try:
                kwargs = self.service_kwargs.copy() if self.service_kwargs is not None else {}
                ...
                async with self._client("ecs") as client:
                    ### TODO
                    docker = DockerSource()
                    definition = DeploymentDefinition(docker=docker)
                    service = CreateService(self.app_id, definition)
                    ServicesApi.create_service(service)
...
class KoyebCluster(SpecCluster, ConfigMixin):
